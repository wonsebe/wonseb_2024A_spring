...
function doAuthCode(){
        ...
        //2.*임의의 인증번호(JS에서 인증번호를 관리하지 않는 이유: JS 는 클라이언트로부터 오픈코드 이기 때문)
        $.ajax({
            async: false ,
            method: "post" , 
            url : '/auth/check' ,
        ...
        if(result){
            ...
            //회원가입을 헀을 때 유효성검사를 하는데 있어서 모두 다 통과해야 회원가입이 될 수 있도록 설정
        }
            }
        })

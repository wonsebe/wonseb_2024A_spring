...
public class FileServise {
    //[0]
    String  uploadPath = "C:\\Users\\tj-bu-703-15\\Desktop\\wonseb_2024A_spring\\src\\main\\resources\\static\\upload";
    ...
    @Autowired private HttpServletRequest request; //HTTP 요청 들어온 정보 와 기능이 포함된 객체
    @Autowired private HttpServletResponse response; //HTTP 응답 할 때 정보 와 기능이 포함된 객체

    
    //[2]파일 다운로드 
    public  void fileDownLoad(String filename){

        System.out.println("FileServise.fileDownLoad");
        System.out.println("filename = " + filename);
        ...
        //해당 다운로드할 파일을 서버(자바)로 바이트를 읽기
            ...
            //- Buffered , 버퍼: 특정 위치로 이동하는 동안 일시적으로 데이터를 보관하는 메모리(스트림)
        try {  //예외
            ///---------------------------------파일을 바이트 배열로 읽어오기---------------------------------///
            //2-1 파일 입력 스트림 객체 생성

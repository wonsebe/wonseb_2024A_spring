/*
	...
	-연산자 
        ...
        4. 기타연산자: 
			1.between 시작값 and 끝값 : 시작값 부터 끝값 까지의 포함된 값
            2. in(값1, 값2)   : 값이 하나라도 포함된 값 조회
            3. [ 필드명 ] like 패턴 : 필드의 패턴 값 검사
				1. % : 모든 문자 대응
                2. _ : _개수 만큼 문자수 대응
			4.[ 필드명 ]  is null, [ 필드명 ] is not null : null : 필드의 null 여부 검사
	...
	-제약조건
        ...
        [6] foreign key : 참조키 ( 다른 테이블의 식별키를 참조하는 필드 )
        -제약조건 옵션
		...
		[정수]
            ...
            BIGINT : 8바이트
				-signed:[부호있음]  음수 사용 ( 기본값 ) 
						-128 ~ 127
                -unsigned :[부호없음] 음수 사용 안함 ( 양수 2배 사용 ) 
						0~255
        [실수]
			FLOAT : 4바이트
            DOUBLE : 8바이트
				DECIMAL : 실수의 오차 없이 표현 가능
